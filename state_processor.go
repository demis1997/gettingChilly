...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (*state.StateDB, types.Receipts, []*types.Log, uint64, error) {
	...
	for i, tx := range block.Transactions() {
		...
		//this is what is being used to get the transaction details including the usedgas which is what i am modifying 
...
func applyTransaction(msg types.Message, config *params.ChainConfig, bc ChainContext, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, evm *vm.EVM, receiptProcessors ...ReceiptProcessor) (*types.Receipt, error) {
	//setting burn address to 0 address
    ...
    // Figure out what the transaction fee is by multiplying fas used by gas price then setting burn amount to 50% and then substracting the burn amount from the total amount to get the remaining fee
    txFee := new(big.Int).Mul(new(big.Int).SetUint64(result.UsedGas), msg.GasPrice())
    burnAmount := new(big.Int).Div(txFee, big.NewInt(50))
    newfee := new(big.Int).Sub(txFee, burnAmount)

    // Now we update the state with the new fee we have
    if author != nil {
        statedb.AddBalance(*author, newfee)
    }

 	//Burning the fee here by removing it from the senders address and adding it to the 0 address
    statedb.SubBalance(msg.From(), burnAmount) 
    statedb.AddBalance(burnAddress, burnAmount) 
